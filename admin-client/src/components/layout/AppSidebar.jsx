            ...
            {isLoading ? (
              ...
              <div className="relative flex items-center">
                <select
                  ...
                  className="w-full bg-transparent text-sm font-semibold text-foreground border-0 py-2 pl-2 pr-8 rounded-xl focus:outline-none focus:ring-1 focus:ring-primary appearance-none cursor-pointer hover:bg-muted/50 transition-colors"
                >
                  {workspaces.map((ws) => (
                    <option key={ws.id} value={ws.id} className="bg-background text-foreground text-xs py-2">
                      {ws.name} ({ws.role})
                    </option>
                  ))}
                </select>
